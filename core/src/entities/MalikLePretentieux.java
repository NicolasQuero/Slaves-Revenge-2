...
public class MalikLePretentieux extends Entity {
	...
	public MalikLePretentieux(float x, float y, GameMap map, boolean ent_played, boolean is_npc) {
		super(x, y, PLAYER_WIDTH - 28, PLAYER_HEIGHT - 2, EntityType.MALIKLEPRETENTIEUX, map, ent_played, PLAYER_WIDTH, PLAYER_HEIGHT, PLAYER_ANIM_SPEED_RUN);
		super.getRekt().setPosition(x + 14, y + 2); // the substractions on top and this line adjust the collision rectangle for the player
		this.setIcon("icons/malik_le_pret_icon.png");
		this.setIs_npc(is_npc);
		player_img = new Animation[5];
		
		TextureRegion[][] playerSpriteSheet = TextureRegion.split(new Texture("characters/malik_le_pretentieux_psg.png"), 32, 32);

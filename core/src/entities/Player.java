...
public class Player extends Entity {
	...
	public Player(float x, float y, GameMap map, boolean ent_played, boolean is_npc) {
		super(x, y, PLAYER_WIDTH - 28, PLAYER_HEIGHT - 48, EntityType.PLAYER, map, ent_played, PLAYER_WIDTH, PLAYER_HEIGHT, PLAYER_ANIM_SPEED_RUN);
		super.getRekt().setPosition(x + 14, y + 24); // the substractions on top and this line adjust the collision rectangle for the player
		this.setIcon("icons/monster_1_icon.png");
		this.setIs_npc(is_npc);
		player_img = new Animation[5];
		
		TextureRegion[][] playerSpriteSheet = TextureRegion.split(new Texture("characters/monster_1.png"), 32, 32);
		...
		this.ATK = 10; this.DEF = 10; this.maxHP = 1000; this.HP = maxHP; this.maxMP = 100; this.MP = maxMP;
		
		spells = new Spell[4];
		spells[0] = new Fireball();
		spells[1] = new HealZone1();
		spells[2] = new FireAttack1();
		spells[3] = new VampExplosion();
